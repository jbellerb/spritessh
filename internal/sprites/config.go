...
// from the system keyring. If currentUser is non-empty, uses a user-specific
// keyring service.
...
// ResolveToken resolves the relevant API token from the global Sprites config.
...
// ResolveTokenWithConfig resolves the relevant API token from the provided
// config.
